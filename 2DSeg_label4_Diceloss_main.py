...
class SegDataset(Dataset):
  def __init__(self,img_path,img_labels,mode=None,transform_x =None,seed = None):
  ...
  def __getitem__(self, idx):
    ...
    label_tensor[(img[:, :, 0] == 0) & (img[:, :, 1] == 128) & (img[:, :, 2] == 0)] = 3 #RGB순으로 0,1,2
    ...
    img = label_to_one_hot_label(label, num_classes=4)#torch.Size([4, 704, 1280])


#     img = img.permute(1,2,0)
#     plt.imshow(img[:,:,2])
#     plt.show()
#     label = label_to_one_hot_label(li1, num_classes=1)#torch.Size([4, 704, 1280])
#     label = torch.tensor(label_tensor)#numpy여서 tensor로 변경#torch.Size([704, 1280])
...
val_y_transforms = transforms.Compose([
    transforms.Resize((320,352)),
#     transforms.Normalize((0.5000006 ,0.5000006 ,0.5000006),(0.4998962 ,0.49989358 ,0.49989018)),
     ])    
...
train_dataset = SegDataset(train_img_path,train_img_label,'train',transform_x=train_x_transforms)#batch 16일 때 이미지 1개씩 SegDataset에 들어감
train_loader = DataLoader(train_dataset,batch_size=6)


#transform normalize()를 하기 위해 rgb 각 channel의 mean과 std
# # RGB별로 평균,표준 편차

# meanRGB = [np.mean(label.numpy(), axis=(1,2)) for x,label in train_loader]
# stdRGB = [np.std(label.numpy(), axis=(1, 2)) for x,label in train_loader]
...
# # 60.62533 63.03786 60.730656
# # 47.828457 49.139538 47.72335
# print(meanR, meanG, meanB)
# print(stdR, stdG, stdB)



val_img_path = "D:/NT data/NT_GT_230720/val/NT_image/NT_VG"
val_img_label ="D:/NT data/NT_GT_230720/val/NT_label/label_NT_VG"

val_dataset = SegDataset(val_img_path,val_img_label,'val',transform_x=val_x_transforms )
val_loader = DataLoader(val_dataset,batch_size=1)


test_img_path = "D:/NT_GT_230720/val/NT_image/NT_VG"
test_img_label ="D:/NT_GT_230720/val/NT_label/label_NT_VG"

test_dataset = SegDataset(test_img_path,test_img_label,'test',transform_x=val_x_transforms )
...
for e  in range(0,n_epochs):
   #train_model
      ...
      label_mask.save('C:/Users/DaHaeLee/medical/label4/'+str(cnt2)+'epoch_train_img_label_.png','PNG')
      if cnt2 == 1:
            target.save('C:/Users/DaHaeLee/medical/label4/'+str(cnt2)+'epoch_train_target_label_.png','PNG')
...
#--------------------------------------------------------------------------------------------------------------Validation----------------------------------------------------------------------------------------------------------------------
      ...
      torch.save(model.state_dict(),'2D_segmentation_label4_5epoch.pt')

# --------------------------------------------------------------------------------------------------------------Test----------------------------------------------------------------------------------------------
model.load_state_dict(torch.load("2D_segmentation_label4_5epoch.pt"))
...
with torch.no_grad():
      ...
      for images, labels in test_loader:
            cnt2 += 1
            images,labels = images.to('cpu'), labels.to('cpu')#images [1,3,256,256]
 ...
 #-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------           
            ...
            target_ = labels#torch.Size([1, 256, 256, 2])
            ...
            cnt3 += 1
            label_mask.save('C:/Users/DaHaeLee/medical/test_5epoch_img_label4_/'+str(cnt3)+'epoch_test1_img_label.png','PNG')
...
# axes[0,0].imshow(pred_dict['input'])
# axes[0,0].set_ylabel('input')

# axes[1,0].imshow(pred_dict['target'])
# axes[1,0].set_ylabel('target')

# axes[2,0].imshow(pred_dict['output'])
# axes[2,0].set_ylabel('output')

# plt.tight_layout()
# plt.savefig(str(cnt2)+'.png')
